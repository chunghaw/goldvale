...
2. **Conversation** — owner text bubble (right, sage); owner bubble with a **photo thumbnail**; a Goldvale reply embedding a **"✓ Logged to Oscar's journal"** chip + a **pattern-memory recall card**; a second reply with an **"＋ Added to vet brief"** chip; and the **typing indicator** (three pulsing dots).
3. **Red flag** — owner describes something worrying; Goldvale replies with the bounded **"Contact your vet now"** alert card (danger) and a calm, non-diagnostic line.

## Rich cards (embedded in Goldvale bubbles)
...
| Goldvale bubble (left, white) | `chat_messages` role `assistant` | streamed from Bedrock; `narrateSafe` |
...
| Text field + send | inserts `chat_messages` | placeholder "Tell Goldvale about Oscar…" |
...
- Disclaimer: "Goldvale remembers and prepares — it doesn't diagnose. Your vet decides."
